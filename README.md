...
- **效能監控**:
  - 生成完成後，結果下方會顯示詳細的時間統計，格式為: `Total: 10.5s (Q: 0.5s, E: 10.0s)`。
  - **Q (Queue)**: 任務在伺服器排隊等待執行的時間。
  - **E (Exec)**: Gemini 模型實際處理並回傳的耗時 (含網路傳輸)。
...
- **結果顯示**: 右側為生成結果 (OUT)。
- **時間資訊**: 每個項目的日期下方皆標註了生成耗時，方便追蹤效能。
- 點擊圖片可放大檢視並下載。

## 資料庫遷移 (Database Migration)

為了支援新的時間追蹤功能，資料庫新增了 `started_at` 與 `completed_at` 欄位。

- **自動遷移**: 系統啟動時會自動檢查並更新資料庫 schema (`backend/migrate_db.py`)。
- **手動遷移**: 若有需要，您也可以手動執行遷移腳本：
  ```bash
...
### 效能追蹤系統 (2025-12-23)
- **新增時間戳記**: 資料庫與 API 現在完整記錄任務的三個時間點：
  - `created_at`: 任務建立時間。
  - `started_at`: 背景任務開始執行時間。
  - `completed_at`: Gemini 回應並完成處理時間。
- **前端顯示**: 介面新增了詳細的耗時統計 (Queue vs Execution)，幫助釐清是伺服器排隊還是模型運算的瓶頸。
...
- `GET /api/generations/{id}`: 取得單次生成的詳細資訊，包含以下時間欄位：
  - `created_at`: 建立時間。
  - `started_at`: 開始執行時間。
  - `completed_at`: 完成時間。
