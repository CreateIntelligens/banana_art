            ...
            <div className="space-y-8 max-w-4xl mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-gray-50 border rounded-xl p-6 space-y-4 h-fit sticky top-24">
                        ...
                        <div> <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Reference Images ({newTemplateImages.length})</label> <div className="grid grid-cols-4 gap-2 h-40 overflow-y-auto p-2 pb-2 border rounded-xl bg-white custom-scrollbar content-start"> <div onClick={() => fileInputRef.current?.click()} className="relative w-full h-0 pb-[100%] border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-100"> <div className="absolute inset-0 flex items-center justify-center"> <Upload size={16} className="text-gray-400" /> <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept="image/*" /> </div> </div> {uploadedImages.map(img => { const selectionIndex = newTemplateImages.indexOf(img.id); const isSelected = selectionIndex !== -1; return ( <div key={img.id} onClick={() => handleImageToggle(img.id, setNewTemplateImages)} className={`relative w-full h-0 pb-[100%] rounded-lg overflow-hidden cursor-pointer border-2 ${isSelected ? 'border-yellow-400 ring-2 ring-yellow-200' : 'border-transparent opacity-70 hover:opacity-100'}`}> <img src={img.filepath} className="absolute inset-0 w-full h-full object-cover block" /> {isSelected && ( <div className="absolute inset-0 flex items-center justify-center bg-black/40"> <div className="w-6 h-6 rounded-full bg-yellow-400 border-2 border-white flex items-center justify-center text-black font-bold text-xs shadow-lg"> {selectionIndex + 1} </div> </div> )} </div> ); })} </div> </div>
